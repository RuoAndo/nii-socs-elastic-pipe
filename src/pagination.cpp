    ...
    std::string         * scroll_id)
{
    ...
    FILE *fp;
    fp = fopen("dump.txt", "ab");

    static char                       buffer[WRITE_BUF_SIZE];
    static rapidjson::FileWriteStream stream(fp, buffer, sizeof(buffer));
...
void pagination(
    pagination_options const& options,
    thread_state      * state)
{
    ...
    char * argv[])
{
    ...
    if (cmdl["--pagination-mappings"])
    {
        return pagination_mappings(
            host,
            index,
            auth);
    }
    ...
    for (int i = 0; i < slices; i++)
    {
        ...
        cnt->slice_id  = i;
        cnt->thread    = std::thread(pagination, opts, &cnt->state);
